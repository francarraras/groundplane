...
async function main() {
  ...
  const resolutionPath = requireArg(args, "resolution");
  ...
  const resolution = await readJson(resolvePath(resolutionPath), "resolution");
  const absoluteQueuePath = resolvePath(queuePath);
  const queue = validateReviewQueue(await readJson(absoluteQueuePath, queuePath));
  const nextQueue = applyReviewResolution(queue, resolution, {
    resolvedAt: args["resolved-at"],
  });
  const item = nextQueue.reviews.find((review) => review?.id === resolution.review_id);
